object Day7 {
    ...
    private val abababCanMatch1 = """^(?:.*])?(?!.*\[.*).*([a-z])(!?\1)([a-z])\1(?:.*\[[^\]]*])*.*\[(?!.*].*).*\2\1\2""".toRegex()
    private val abababCanMatch2 = """^.*\[(?!.*].*).*([a-z])(?!\1)([a-z])\1.*](?:.*\[[^\]]*])*(?!.*\[.*).*\2\1\2""".toRegex()

    fun part1() = input.filter { abbaMustMatch.containsMatchIn(it) && !abbaMustNotMatch.containsMatchIn(it) }.count()

    fun part2() = input.filter { abababCanMatch1.containsMatchIn(it) || abababCanMatch2.containsMatchIn(it) }.count()
}
